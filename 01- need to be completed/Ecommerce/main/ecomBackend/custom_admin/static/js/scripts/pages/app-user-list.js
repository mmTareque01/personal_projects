			...
			columnDefs: [
				...
				},
				{
					...
					render: function (data, type, full, meta) {
						...
						var $row_output = '<div class="d-flex justify-content-left align-items-center">' + '<div class="avatar-wrapper">' + '<div class="avatar ' + colorClass + ' me-1">' + $output + "</div>" + "</div>" + '<div class="d-flex flex-column">' + '<a href="' + userView + '" class="user_name text-truncate text-body"><span class="fw-bolder">' + $name + "</span></a>" + '<small class="emp_post text-muted">' + $email + "</small>" + "</div>" + "</div>";
						return $row_output;
					},
				},
				{
			...
			initComplete: function () {
				...
				this.api()
					...
					.every(function () {
						...
						var select = $('<select id="UserPlan" class="form-select text-capitalize mb-md-0 mb-2"><option value=""> Select Plan </option></select>')
							.appendTo(".user_plan")
							.on("change", function () {
								var val = $.fn.dataTable.util.escapeRegex($(this).val());
								column.search(val ? "^" + val + "$" : "", true, false).draw();
				...
				this.api()
					...
					.every(function () {
						...
						var select = $('<select id="FilterTransaction" class="form-select text-capitalize mb-md-0 mb-2xx"><option value=""> Select Status </option></select>')
