			...
			columnDefs: [
				...
				},
				{
					...
					render: function (data, type, full, meta) {
						...
						var $row_output = '<div class="d-flex justify-content-left align-items-center">' + '<div class="avatar-wrapper">' + '<div class="avatar ' + colorClass + ' me-1">' + $output + "</div>" + "</div>" + '<div class="d-flex flex-column">' + '<a href="' + userView + '" class="user_name text-body text-truncate"><span class="fw-bolder">' + $name + "</span></a>" + '<small class="emp_post text-muted">' + $email + "</small>" + "</div>" + "</div>";
						return $row_output;
					},
				},
				{
				...
				},
				{
					...
					orderable: false,
					render: function (data, type, full, meta) {
						return '<a href="' + userView + '" class="btn btn-sm btn-icon">' + feather.icons["eye"].toSvg({ class: "font-medium-3 text-body" }) + "</a>";
					},
				},
			],
			order: [[2, "desc"]],
