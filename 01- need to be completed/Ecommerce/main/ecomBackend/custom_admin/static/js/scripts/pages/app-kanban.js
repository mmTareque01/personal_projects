$(function () {
	...
	function renderHeader(color, text) {
		return "<div class='d-flex justify-content-between flex-wrap align-items-center mb-1'>" + "<div class='item-badges'> " + "<div class='badge rounded-pill badge-light-" + color + "'> " + text + "</div>" + "</div>" + renderDropdown() + "</div>";
	}
		...
		return images !== undefined
			? images
					...
					.map(function (img, index, arr) {
						...
						return "<li class='avatar kanban-item-avatar" + " " + $transition + " " + $margin + "'" + "data-bs-toggle='tooltip' data-bs-placement='top'" + "title='" + member[index] + "'" + ">" + "<img src='" + assetPath + "images/portrait/small/" + img + "' alt='Avatar' height='" + size + "'>" + "</li>";
					})
					.join(" ")
			: "";
	}
	...
	var kanban = new jKanban({
		...
		click: function (el) {
			...
			sidebar.find(".assigned").append(renderAvatar(avatars, false, "50", el.attr("data-members"), 32) + "<li class='avatar avatar-add-member ms-50'>" + "<span class='avatar-content'>" + feather.icons["plus"].toSvg({ class: "avatar-icon" }) + "</li>");
		},
		buttonClick: function (el, boardId) {
			var addNew = document.createElement("form");
			addNew.setAttribute("class", "new-item-form");
			addNew.innerHTML = '<div class="mb-1">' + '<textarea class="form-control add-new-item" rows="2" placeholder="Add Content" required></textarea>' + "</div>" + '<div class="mb-2">' + '<button type="submit" class="btn btn-primary btn-sm me-1">Add</button>' + '<button type="button" class="btn btn-outline-secondary btn-sm cancel-add-item">Cancel</button>' + "</div>";
