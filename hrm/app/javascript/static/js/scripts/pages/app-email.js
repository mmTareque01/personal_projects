...
$(function () {
	...
	function renderGuestAvatar(option) {
		...
		var $avatar = "<div class='d-flex flex-wrap align-items-center'>" + "<div class='avatar avatar-sm my-0 me-50'>" + "<span class='avatar-content'>" + avatarImg + "</span>" + "</div>" + option.text + "</div>";
	...
	$(document).on("click", ".email-app-list .selectAll input", function () {
		if ($(this).is(":checked")) {
			userActions.find(".form-check .form-check-input").prop("checked", this.checked).closest(".user-mail").addClass("selected-row-bg");
		} else {
			userActions.find(".form-check .form-check-input").prop("checked", "").closest(".user-mail").removeClass("selected-row-bg");
		}
	});
