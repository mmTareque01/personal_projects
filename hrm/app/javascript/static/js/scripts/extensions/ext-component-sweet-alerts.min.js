$(function () {
	...
	"laravel" === $("body").attr("data-framework") && (x = $("body").attr("data-asset-path")),
		...
		c.length &&
			c.on("click", function () {
				Swal.fire({ title: "Bounce In Animation", customClass: { confirmButton: "btn btn-primary" }, showClass: { popup: "animate__animated animate__bounceIn" }, buttonsStyling: !1 });
			}),
		r.length &&
			r.on("click", function () {
				Swal.fire({ title: "Fade In Animation", customClass: { confirmButton: "btn btn-primary" }, showClass: { popup: "animate__animated animate__fadeIn" }, buttonsStyling: !1 });
			}),
		u.length &&
			u.on("click", function () {
				Swal.fire({ title: "Flip In Animation", customClass: { confirmButton: "btn btn-primary" }, showClass: { popup: "animate__animated animate__flipInX" }, buttonsStyling: !1 });
			}),
		m.length &&
			m.on("click", function () {
				Swal.fire({ title: "Tada Animation", customClass: { confirmButton: "btn btn-primary" }, showClass: { popup: "animate__animated animate__tada" }, buttonsStyling: !1 });
			}),
		f.length &&
			f.on("click", function () {
				Swal.fire({ title: "Shake Animation", customClass: { confirmButton: "btn btn-primary" }, showClass: { popup: "animate__animated animate__shakeX" }, buttonsStyling: !1 });
			}),
		b.length &&
			b.on("click", function () {
		...
		w.length &&
			w.on("click", function () {
				Swal.fire({ title: "Sweet!", text: "Modal with a custom image.", imageUrl: x + "images/slider/04.jpg", imageWidth: 400, imageHeight: 200, imageAlt: "Custom image", customClass: { confirmButton: "btn btn-primary" }, buttonsStyling: !1 });
			}),
		d.length &&
			d.on("click", function () {
				var t;
		...
		S.length &&
			S.on("click", function () {
				Swal.fire({ title: "Click outside to close!", text: "This is a cool message!", customClass: { confirmButton: "btn btn-primary" }, buttonsStyling: !1 });
			}),
		y.length &&
			y.on("click", function () {
				const t = ["1", "2", "3"],
					n = Swal.mixin({ confirmButtonText: "Forward", cancelButtonText: "Back", progressSteps: t, input: "text", inputAttributes: { required: !0 }, validationMessage: "This field is required" });
				!(async function () {
					const o = [];
					let i;
					for (i = 0; i < t.length; ) {
		...
		B.length &&
			B.on("click", function () {
				Swal.fire({
					...
					t.isConfirmed && Swal.fire({ title: t.value.login + "'s avatar", imageUrl: t.value.avatar_url, customClass: { confirmButton: "btn btn-primary" } });
				});
			}),
		C.length &&
			C.on("click", function () {
				Swal.fire({ title: "Are you sure?", text: "You won't be able to revert this!", icon: "warning", showCancelButton: !0, confirmButtonText: "Yes, delete it!", customClass: { confirmButton: "btn btn-primary", cancelButton: "btn btn-outline-danger ms-1" }, buttonsStyling: !1 }).then(function (t) {
		...
		k.length &&
			k.on("click", function () {
				Swal.fire({ title: "Are you sure?", text: "You won't be able to revert this!", icon: "warning", showCancelButton: !0, confirmButtonText: "Yes, delete it!", customClass: { confirmButton: "btn btn-primary", cancelButton: "btn btn-outline-danger ms-1" }, buttonsStyling: !1 }).then(function (t) {
					t.value ? Swal.fire({ icon: "success", title: "Deleted!", text: "Your file has been deleted.", customClass: { confirmButton: "btn btn-success" } }) : t.dismiss === Swal.DismissReason.cancel && Swal.fire({ title: "Cancelled", text: "Your imaginary file is safe :)", icon: "error", customClass: { confirmButton: "btn btn-success" } });
				});
			});
});
